...
public class MergeTemplate {
    public static void main(String[] args) throws Exception {
        ...
        if (!tmpFile.exists()) {
            showHelp(
                    "The template file " + tmpFile.getAbsolutePath() + " does not exist!", options);
            return;
        }
        ...
        while (it.hasNext()) {
            ...
            if (!CommonUtils.isEmpty(value) && value.toLowerCase().startsWith("json_file:")) {
                ...
                if (vals == null || vals.length < 2) {
                    throw new Exception(
                            "Please confirm your key format "
                                    + value
                                    + "  in data file, key->json(key=json_file:data.txt)");
                }
                ...
                if (!jsonData.exists()) {
                    throw new Exception(
                            "Please confirm your json data file "
                                    + jsonData.getAbsolutePath()
                                    + " exists!");
                }

                String jsonDataContent =
                        CommonUtils.getFileContent(jsonData.getAbsolutePath(), false);
                templateFileContents =
                        CommonUtils.replace(
                                templateFileContents,
                                "#" + key + "#",
                                CommonUtils.getJson(jsonDataContent));
                ...
                if (vals == null || vals.length < 2) {
                    throw new Exception(
                            "Please confirm your key format "
                                    + value
                                    + "  in data file, key->file(key=file:data.txt)");
                }
                ...
                if (!fdata.exists()) {
                    throw new Exception(
                            "Please confirm your data file "
                                    + fdata.getAbsolutePath()
                                    + " exists!");
                }
    ...
    private static void showHelp(String error, Options options) {
        ...
        formatter.printHelp(
                "MergeTemplate <-t|--template-file> template-file <-d|--data-file> data-file <-o|--output-file> out-file",
                options);
        System.out.println();
    }
}
